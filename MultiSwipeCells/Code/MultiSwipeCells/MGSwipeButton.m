...
+(instancetype) buttonWithTitle:(NSString *) title icon:(UIImage*) icon backgroundColor:(UIColor *) color titileColor:(UIColor *) titlecolor  imageInsets:(UIEdgeInsets) imageInsets titleInsets:(UIEdgeInsets) titleInsets  Tag:(NSIndexPath*) tag  callback:(MGSwipeButtonCallback) callback
{
    MGSwipeButton * button = [self buttonWithType:UIButtonTypeCustom];
    button.backgroundColor = color;
    button.titleLabel.lineBreakMode = NSLineBreakByWordWrapping;
    ...
    [button setImage:icon forState:UIControlStateNormal];
    button.callback = callback;
    button.titleLabel.font = [UIFont fontWithName:@"SFUIDisplay-Regular" size:16];
    button.tag = tag.row;
