...
int const swipeButton_height = 121;
...
-(instancetype) initWithButtons:(NSArray*) buttonsArray direction:(MGSwipeDirection) direction differentWidth:(BOOL) differentWidth
{
    ...
    for (UIView * button in buttonsArray) {
        ...
        maxSize.height = swipeButton_height;//MAX(maxSize.height, button.bounds.size.height);
    }
    if (!differentWidth) {
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, containerWidth, maxSize.height)]) {
        ...
        frame.size.height = swipeButton_height;
        frame.origin.x = 0;
...
-(void) resetButtons
{
    CGFloat offsetX = 0;
    for (UIView * button in _buttons) {
        button.frame = CGRectMake(offsetX, 0, button.bounds.size.width, swipeButton_height); //_container.bounds.size.height
        button.autoresizingMask = UIViewAutoresizingFlexibleHeight;
...
-(void) layoutExpansion: (CGFloat) offset
{
    ...
    _container.frame = CGRectMake(_fromLeft ? 0: self.bounds.size.width - offset, 0, offset,swipeButton_height );// self.bounds.size.height);
...
-(void) layoutSubviews
{
    ...
    if (_expandedButton) {
        ...
        _container.frame = CGRectMake(self.bounds.origin.x, self.bounds.origin.y, self.bounds.size.width, self.bounds.size.height);//self.bounds;
    }
}
...
-(CGRect) expansionBackgroundRect: (UIView *) button
{
    ...
    if (_fromLeft) {
        return CGRectMake(-extra, 0, button.frame.origin.x + extra, swipeButton_height); //_container.bounds.size.height
    }
    else {
        return CGRectMake(button.frame.origin.x +  button.bounds.size.width, 0,
                          ...
                          ,swipeButton_height); //_container.bounds.size.height
    }
}
...
-(void) initViews: (BOOL) cleanButtons
{
    ...
    big_bg.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    big_bg.contentMode = UIViewContentModeCenter;
}
...
//        [self.contentView addSubview:_swipeContentView];
        [big_bg addSubview:_swipeContentView];
    }
...
-(void) layoutSubviews
{
    ...
    [_swipeOverlay setBackgroundColor:[UIColor clearColor]];
...
{
    if (!_swipeOverlay) {
        _swipeOverlay = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, swipeButton_height)]; //self.bounds.size.height
        _swipeOverlay.hidden = YES;
        _swipeOverlay.backgroundColor = [UIColor clearColor]; //[self backgroundColorForSwipe];
        _swipeOverlay.layer.zPosition = 8; //force render on top of the contentView;
        ...
        [big_bg addSubview:_swipeOverlay];
...
//    UIGraphicsBeginImageContextWithOptions(view.bounds.size, NO, [[UIScreen mainScreen] scale]);
    ...
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(view.bounds.size.width, swipeButton_height) , NO, [[UIScreen mainScreen] scale]);
...
-(void) setAccesoryViewsHidden: (BOOL) hidden
{
    ...
    for (UIView * view in big_bg.superview.subviews) {
    ...
    for (UIView * view in big_bg.subviews) {
...
- (void)setSwipeOffset:(CGFloat) newOffset;
{
    ...
    __block MGSwipeExpansionSettings *expansions[2] = {_leftExpansion, _rightExpansion};
    ...
    for (int i = 0; i< 2; ++i)
    {
        ...
        if (expand)
        {
#pragma changes made for long elastic after reaching to end then refresh swipe buttons
//            NSLog(@"expand offset %f",offset);
             ...
             if (newOffset > 0 && newOffset > 200)
            {
             ...
             else if ((newOffset <= -195 && newOffset >= -200) || (newOffset >= 195 && newOffset <= 200))
             {
                 [view expandToOffset:90 settings:expansions[i]];

                 [UIView animateWithDuration:0.3 animations:^{

                     _targetOffset = 90;//expansions[i].fillOnTrigger ? self.bounds.size.width * sign : 0;
                     _activeExpansion = view;
            ...
            else if (newOffset < 0 && newOffset < -200)
            {
                [view expandToOffset:offset settings:expansions[i]];
                _targetOffset = expansions[i].fillOnTrigger ? self.bounds.size.width * sign : 0;
                _activeExpansion = view;
...
-(void)setSwipeBackgroundColor:(UIColor *)swipeBackgroundColor {
    _swipeBackgroundColor = [UIColor clearColor];
}
